...
// Returns the sum of all numbers 0 -> distance
function fuel_distance(distance) {
    var calculated = 0;
...
var cheapest_option = max*fuel_distance(max);
...
for (let test_pos = 0; test_pos < horizontal_array.length; test_pos++) {
    ...
    for (let crab_i = 0; crab_i < horizontal_array.length; crab_i++) {
        ...
        const distance = fuel_distance(Math.abs(crab_i - test_pos));
